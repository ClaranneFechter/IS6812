...
## Description of Data Cleaning and Modeling Process
...
```{r join train, , echo=TRUE, results='hide'}
...
```{r join test, , echo=TRUE, results='hide'}
# Now lets join the datasets
...
```{r final clean test, , echo=TRUE, results='hide'}
...
For handling outliers, we have decided to limit the dataset by using the interquartile method. We are trying to focus mostly on the middle 50 percent of the numeric columns. We are capping each numeric column, except for TARGET, using the ranges of our lower and upper bounds. Everything outside of this is adjusted to be within the range. By doing this, we should see improvements in our models performance and improve the quality of our data.
...
```{r base model, , echo=TRUE, results='hide'}
...
It will not let me see leader board position.
...
# model using predictors that typical banks might look at
...
It will not let us see leader board position. 
...
# Model with high signifance and what other banks might use
...
summary(model3)$coefficients[order(summary(model3)$coefficients[,4]), ]  # Sort by p-value
```
...
Our AUC for this model is 0.7473. This is our highest AUC of the three models we have made. This shows that we are on the right track and our score is improving.
...
We can also see that the Kaggle score confirms that we are improving our model.

### Logistic Regression Summary

Model 1: AUC = 0.7427 Kaggle Score: Public Score: 0.73214 Private Score: 0.72993

Model 2: AUC = 0.7432 Kaggle Score: Public Score: 0.73524 Private Score: 0.72926

Model 3: AUC = 0.7473 Kaggle Score: Public Score: 0.73880 Private Score: 0.73291
...
Lets now try a different model type, naive bayes. This model works well when predictors are independent.
...
AUC = 0.7107 Kaggle: Public Score: 0.52578 Private score: 0.52564
...
AUC: 0.6959366 Kaggle: Public Score: 0.64202 Private score: 0.64318
...
Claranne Fechter - Logistic and NB modeling, Outlier handling, final data cleaning, final file cleaning, submissions, presentation 
